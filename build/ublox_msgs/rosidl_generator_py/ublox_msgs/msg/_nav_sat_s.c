...
bool ublox_msgs__msg__nav_satsv__convert_from_py(PyObject * _pymsg, void * _ros_message);
PyObject * ublox_msgs__msg__nav_satsv__convert_to_py(void * raw_ros_message);

ROSIDL_GENERATOR_C_EXPORT
bool ublox_msgs__msg__nav_sat__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ublox_msgs.msg._nav_sat.NavSAT", full_classname_dest, 30) == 0);
  }
  ublox_msgs__msg__NavSAT * ros_message = _ros_message;
  {  // i_tow
  ...
  {  // sv
    ...
    if (!ublox_msgs__msg__NavSATSV__Sequence__init(&(ros_message->sv), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create ublox_msgs__msg__NavSATSV__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    ublox_msgs__msg__NavSATSV * dest = ros_message->sv.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!ublox_msgs__msg__nav_satsv__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of NavSAT */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("ublox_msgs.msg._nav_sat");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "NavSAT");
  ...
  {  // sv
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = ublox_msgs__msg__nav_satsv__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }

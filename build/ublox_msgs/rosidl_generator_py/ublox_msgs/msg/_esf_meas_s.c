...
bool ublox_msgs__msg__esf_meas__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ublox_msgs.msg._esf_meas.EsfMEAS", full_classname_dest, 32) == 0);
  }
  ublox_msgs__msg__EsfMEAS * ros_message = _ros_message;
  {  // time_tag
  ...
  {  // calib_t_tag
    PyObject * field = PyObject_GetAttrString(_pymsg, "calib_t_tag");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      uint32_t * dest = ros_message->calib_t_tag.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'calib_t_tag'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of EsfMEAS */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("ublox_msgs.msg._esf_meas");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "EsfMEAS");
  ...
  {  // calib_t_tag
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "calib_t_tag");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->calib_t_tag.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint32_t * src = &(ros_message->calib_t_tag.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->calib_t_tag.size * sizeof(uint32_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);

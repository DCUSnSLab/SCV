  ...
  void * untyped_ros_message)
{
  ...
  // Field name: extension
  {
    ...
    if (!ublox_msgs__msg__MonVERExtension__Sequence__init(&ros_message->extension, size)) {
      fprintf(stderr, "failed to create array for field 'extension'");
      return false;
    }
    auto array_ptr = ros_message->extension.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = ublox_msgs__msg__MonVER;
    is_plain =
      (
      offsetof(DataType, extension) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _MonVER__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_ublox_msgs__msg__MonVER(

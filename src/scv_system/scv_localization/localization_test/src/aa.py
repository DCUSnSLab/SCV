...
for i, (bag_file, topic_name) in enumerate(bag_files):
    ...
    print(f"  -> 평균 오차: {mean_error:.2f} m, 최대 오차: {max_error:.2f} m, RMSE: {rmse:.2f} m, 90% 오차: {p90_error:.2f} m, 분산: {variance:.4f}")

    results.append((bag_file, mean_error, max_error, rmse, p90_error, variance))

    # 각 odometry 궤적에 대해 고유한 스타일 적용
    color_idx = i % len(colors)
    marker_idx = i % len(markers)
    line_style_idx = (i // len(colors)) % len(line_styles)
    
    # odometry 궤적 플롯 - 파일 타입에 따라 그룹화
    if "dual_gps" in bag_file:
        label_prefix = "Dual GPS"
        zorder = 3
    elif "single_gps" in bag_file:
        label_prefix = "Single GPS"
        zorder = 2
    else:
        label_prefix = "GPS"
        zorder = 1
        
    run_number = bag_file[-5]  # gps1.bag에서 '1' 추출
    ...
    plt.plot(x_list, y_list, 
             ...
             markersize=4,
             markevery=max(1, len(x_list)//30),  # 30개 정도만 표시
...
# 결과 테이블 출력
print("\n==== 전체 결과 ====")
print(f"{'Bag 파일명':<20} {'평균오차(m)':<12} {'최대오차(m)':<12} {'RMSE(m)':<10} {'90%오차(m)':<12} {'분산':<10}")
for bag_file, mean_error, max_error, rmse, p90_error, variance in results:
    print(f"{bag_file:<20} {mean_error:<12.2f} {max_error:<12.2f} {rmse:<10.2f} {p90_error:<12.2f} {variance:<10.4f}")
...
# 그래프 저장 (선택사항)
# plt.savefig("odometry_trajectories_comparison.png", dpi=300, bbox_inches='tight')
